...
//	SetValue((float)value);
	CMFCPropertyGridProperty::SetValue((float)value);
	SetSliderPos();
...
BOOL CPropGridSlider::OnEdit(LPPOINT lptClick)
{	
	return CMFCPropertyGridProperty::OnEdit(lptClick);
}

BOOL CPropGridSlider::OnUpdateValue()
{
	COleVariant value = GetValue();
	if (value.fltVal > 1.0f)
		CMFCPropertyGridProperty::SetValue(1.0f);
	if (value.fltVal < 0.0f)
		CMFCPropertyGridProperty::SetValue(0.0f);
	return TRUE;
}
